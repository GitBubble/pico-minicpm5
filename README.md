...
Two board sessions, played at the speed they actually ran. Nothing is sped up
and nothing is cut, so the numbers on screen are the ones the board produced.

**A tool-calling agent — one model**
...
**A multimodal agent — two models on one NPU**

<img src="docs/media/board-vision.gif" alt="MiniCPM5-1B and MiniCPM-4v-0.5B running together on one Hi3403 board" width="100%">

`描述一下 chart.png` is routed in `3.1 ms` and handed to MiniCPM-4v-0.5B, which
runs in its own process behind a job queue. The description builds on the
prompt line while MiniCPM5-1B stays available, and lands `21.4 s` later without
the user pressing a key. The picture it read is
[`docs/media/vision-demo.png`](docs/media/vision-demo.png), so the description
...
A second model rides alongside that bundle. MiniCPM-4v-0.5B reads images, and
because each model holds three resident OM handles against one NPU, the two
cannot take turns — they run as separate processes joined by a filesystem job
queue, so posting an image never blocks a conversation. `decode.om` for that
model declares 53 inputs and 49 outputs against this SDK's 32-port cap and
will not load, so tokens are generated by re-running prefill over its 200-row
window with the answer so far appended. Design, measurements and deployment:
[two models on one NPU](docs/MULTIMODAL_VISION.md).
...
The vision skill is board-proven rather than gated: it has no numeric oracle,
because the answer is a sentence. Measured end to end on Hi3403 with a
1440x900 screenshot and a 40-token cap — claimed by the worker at `1.02 s`,
first token visible at `1.98 s`, `0.52 s` per token flat, done at `22.56 s`.
That per-token cost is one whole prefill and does not come down on this SDK;
the queue is what keeps it off the conversation. The 4v model files are
owner-supplied and are not redistributed here.
