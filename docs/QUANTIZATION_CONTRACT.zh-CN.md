...
- PICO ATC 是 IFMR-only。它**无法**摄入 QDQ、AMCT 或 QAT 的 scale——算子表里
...
- 数据通路模式由 `--compile_mode` 选择,而非独立开关。厂商枚举把两个取值命名为
  ...
  `compile_mode=1` 构建记录 `calc_data_type: S16` 配 `weight { S8 }`,而一个
  **另一个模型**的 `compile_mode=0` 构建记录 `S8`/`S8`。不存在同一张图上的
  受控 A/B。
...
无论如何确定的是:钳位是真实的运行时算子,因此标定与推理看到同一个分布——会
...
`>= 1`。已发行的 ctx1024 两个二进制在 687 MB 上仅相差 2,529 字节。
