...
Hi3403 ctx1024 三句柄产物契约。合格产物的大小和 SHA256 位于
...
的 `<tools>/<function>/<param>/<tool_response>` 契约，支持多轮工具回填、会话历史、
...
彩色输入提示符现遵循 readline 的零宽转义契约，中文输入退格到行首时不会残留
第一个字符。
...
runtime profile 将 context、模型产物、能力、生成限制和数值策略绑定为一个契约。
...
`1+1 equals` 产生已验收的 EOS 序列。未声明可信槽位契约的 legacy 调用仍
...
prompt 送入不再为“下一个输入 token 已知、预测会立即丢弃”的位置执行词表 head
